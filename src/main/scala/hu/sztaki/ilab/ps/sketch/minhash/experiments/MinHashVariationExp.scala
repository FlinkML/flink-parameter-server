...
import hu.sztaki.ilab.ps.sketch.minhash.variation.MinHash
...
/**
  ...
  */
class MinHashVariationExp

object MinHashVariationExp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val src = env
      ...
      .flatMap(value => {
        ...
        val tweet = value.split(delimiter)(5).split(" ").map(_.toLowerCase).filter(words.contains(_))
        if (tweet.nonEmpty)
          Some((id, tweet))
        else None
      })
