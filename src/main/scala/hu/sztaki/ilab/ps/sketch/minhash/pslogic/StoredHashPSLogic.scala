...
/**
  ...
  */
class StoredHashPSLogic(numHashes: Int) extends ParameterServerLogic[Int, Long, (Int, Array[Long])] {

  val model = mutable.HashMap.empty[Int, Array[Long]]

  override def onPullRecv(id: Int, workerPartitionIndex: Int, ps: ParameterServer[Int, Long, (Int, Array[Long])]): Unit = ???

  override def onPushRecv(id: Int, tweetId: Long, ps: ParameterServer[Int, Long, (Int, Array[Long])]): Unit =
    model.get(id) match {
      case Some(param) =>
  ...
  override def close(ps: ParameterServer[Int, Long, (Int, Array[Long])]): Unit =
